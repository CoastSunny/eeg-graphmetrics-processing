...
startSubject        = ft_getopt(cfg, 'startNum', 1);
endSubject          = ft_getopt(cfg, 'endNum', 'end');
...
if strcmp(analyses, 'qualityControl')
    analyses =  {'preprocessRaw', 'freqAnalysis', 'artefactDetection', 'detectBadChannels', 'countGoodTrials'};
end
if strcmp(analyses, 'preprocessing')
    analyses =  {'preprocessRaw'};
end
if strcmp(createFigures, 'on');
    analyses = cat(2, analyses, 'createFigures');
end
...
if strcmpi(rmBadChannels, 'off')
    rmBadChannels = false;
elseif strcmpi(rmBadChannels, 'on') 
    rmBadChannels = true;
else
    warning('unknown cfg.rmBadChannels string, not removing channels')
    rmBadChannels = false;
end
...
if nargin == 2
    ...
    PATHS.ROOT      = pwd;
    PATHS.RAW       = [PATHS.ROOT filesep RAWDir];
...
for iRaw = startSubject:endSubject
    ...
    if ~dataInput
        ...
        headerfile = [PATHS.RAW filesep currFilename];
